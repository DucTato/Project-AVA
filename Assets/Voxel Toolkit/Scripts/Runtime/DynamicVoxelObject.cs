...
namespace VoxelToolkit
{
    ...
    public class DynamicVoxelObject : MonoBehaviour
    {
        ...
        private static UnityEngine.Material defaultOpaqueTexturelessMaterial;
        private static UnityEngine.Material defaultTransparentTexturelessMaterial;
        
        private static UnityEngine.Material defaultOpaqueTexturedMaterial;
        private static UnityEngine.Material defaultTransparentTexturedMaterial;
            ...
            for (var x = min.x; x < max.x; x++)
            {
                for (var y = min.y; y < max.y; y++)
                {
                    for (var z = min.z; z < max.z; z++)
                    {
                        ...
                        if (voxel.VoxelKind == VoxelKind.Empty && includeEmpty ||
                            voxel.VoxelKind == VoxelKind.Solid && includeSolids)
        ...
        private void UpdateVoxelObject()
        {
            ...
            if (defaultOpaqueTexturelessMaterial == null)
                defaultOpaqueTexturelessMaterial = Resources.Load<UnityEngine.Material>($"{PathUtility.GetMaterialPath()}/VoxelToolkitDefaultOpaqueTextureless");
            
            if (defaultTransparentTexturelessMaterial == null)
                defaultTransparentTexturelessMaterial = Resources.Load<UnityEngine.Material>($"{PathUtility.GetMaterialPath()}/VoxelToolkitDefaultTransparentTextureless");

            if (defaultOpaqueTexturedMaterial == null)
                defaultOpaqueTexturedMaterial = Resources.Load<UnityEngine.Material>($"{PathUtility.GetMaterialPath()}/VoxelToolkitDefaultOpaqueTextured");
            
            if (defaultTransparentTexturedMaterial == null)
                defaultTransparentTexturedMaterial = Resources.Load<UnityEngine.Material>($"{PathUtility.GetMaterialPath()}/VoxelToolkitDefaultTransparentTextured");

            voxelObject.Scale = scale;
            voxelObject.MeshGenerationApproach = MeshGenerationApproach;
            
            voxelObject.UpdateMeshes(indexFormat, -(float3)voxelObject.Size * 0.5f);
            ...
            for (var index = 0; index < voxelObject.MeshesCount; index++)
            {
                ...
                var defaultOpaque = meshGenerationApproach == MeshGenerationApproach.Textured
                    ? defaultOpaqueTexturedMaterial
                    : defaultOpaqueTexturelessMaterial;
                
                var defaultTransparent = meshGenerationApproach == MeshGenerationApproach.Textured
                    ? defaultTransparentTexturedMaterial
                    : defaultTransparentTexturelessMaterial;
                
                var targetMaterial = mesh.Kind == MeshKind.Opaque ? 
                    (opaqueMaterial == null ? defaultOpaque : opaqueMaterial) :
                    (transparentMaterial == null ? defaultTransparent : transparentMaterial);

                if (meshGenerationApproach == MeshGenerationApproach.Textured)
                {
                    targetMaterial = new UnityEngine.Material(defaultOpaque);
                    defaultOpaque.mainTexture = mesh.ColorTexture;
                }
