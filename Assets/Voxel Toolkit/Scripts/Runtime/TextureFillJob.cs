            ...
            for (var index = 0; index < count; index++)
            {
                ...
                switch (quad.Orientation)
                {
                    case FaceOrientation.Left:
                        size = new int2(quad.EndY - quad.StartY, quad.EndZ - quad.StartZ);
                        for (var z = end.z - 1; z >= start.z; z--)
                        {
                            for (var y = start.y; y < end.y; y++)
                            {
                                ...
                                ColorTarget.Add(material.Color);
                                position++;
                            }  
                        }   
                        break;
                    ...
                    case FaceOrientation.Bottom:
                        size = new int2(quad.EndZ - quad.StartZ, quad.EndX - quad.StartX);
                        for (var x = end.x - 1; x >= start.x; x--)
                        {
                            for (var z = start.z; z < end.z; z++)
                            {
                                var voxelPosition = new int4(x, quad.StartY, z, 0);
                    ...
                    case FaceOrientation.Further:
                        size = new int2(quad.EndY - quad.StartY, quad.EndX - quad.StartX);
                        for (var x = end.x - 1; x >= start.x; x--)
                        {
                            for (var y = start.y; y < end.y; y++)
                            {
                ...
                if (singleColor)
                {
                    ColorTarget.RemoveRange(position + 1, position - originalPosition - 1);
                    size = new int2(1, 1);
