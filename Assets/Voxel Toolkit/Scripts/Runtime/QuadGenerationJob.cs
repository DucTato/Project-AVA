...
namespace VoxelToolkit
{
    ...
    public struct QuadGenerationJob : IJob
    {
        ...
        private int GetLineLength(int4 position, int4 direction, int material, FaceOrientation orientation, int4 size, int chunkSizeSquared)
        {
            ...
            var originMaterialType = originalMaterial.MaterialType;
            for (; math.all(endPosition < size); endPosition += direction)
            {
                ...
                if (originMaterialType != materialData.MaterialType)
                    break;
                
                if (originalMaterial.Parameters != materialData.Parameters)
                    break;
